...
import {
  CLIENTS, ALERTS, INCIDENTS, ALERT_TREND_DATA, SOURCE_DISTRIBUTION,
  CLIENT_ALERT_COUNTS, STATS, SEVERITY_CONFIG,
} from '../data/mockData';
...
/* ── Severity badge (using inline style since classes use custom colours) ── */
...
/* ── Stat Card ── */
function StatCard({ icon: Icon, label, value, sub, color, trend, pulse }: any) {
  return (
    <Card>
      <CardContent className="pt-5">
        ...
        {sub && (
          <p className="flex items-center gap-1 mt-1" style={{ color: '#94A3B8', fontSize: 11 }}>
            {trend === 'up' ? <TrendingUp size={11} style={{ color: '#CB5229' }} /> : trend === 'down' ? <TrendingDown size={11} style={{ color: '#10B981' }} /> : null}
            {sub}
          </p>
        )}
      </CardContent>
    ...
    <div className="rounded-xl shadow-lg p-3 bg-white" style={{ border: '1px solid #E2E8F0', minWidth: 140 }}>
      ...
      {payload.map((e: any) => (
        <div key={e.dataKey} className="flex items-center gap-2 mb-0.5">
          ...
          <span style={{ color: '#475569', fontSize: 11 }}>{e.name}: <strong style={{ color: '#0F172A' }}>{e.value}</strong></span>
        </div>
      ))}
    </div>
  );
...
export default function Dashboard() {
  ...
  const openIncidents = INCIDENTS
    ...
    .filter(i => selectedClient === 'all' || i.clientId === selectedClient);
  ...
  return (
    <div className="p-4 md:p-6 space-y-4 md:space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 md:gap-4">
        <StatCard icon={AlertTriangle} label="Total Alerts" value={STATS.totalAlertsToday} sub="+14% vs yesterday" trend="up" color="#CB5229" />
        <StatCard icon={Zap} label="Critical" value={STATS.criticalAlerts} sub="Immediate action required" color="#CB5229" pulse />
        <StatCard icon={Shield} label="Open Incidents" value={STATS.openIncidents} sub="3 critical, 2 high" color="#F0BC2C" />
        <StatCard icon={Clock} label="Avg MTTR" value={STATS.avgMTTR} sub="↓ 8 min vs last week" trend="down" color="#10B981" />
        <StatCard icon={Activity} label="Events / sec" value={STATS.eventsPerSecond.toLocaleString()} sub="Live ingestion rate" color="#1AABBA" pulse />
        <StatCard icon={Users} label="Clients" value={STATS.clientsMonitored} sub="All systems active" color="#2563EB" />
      </div>
        ...
        <Card className="lg:col-span-2">
          <CardHeader className="pb-2">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
              ...
              <div className="flex flex-wrap gap-3">
                {[{ l: 'Critical', c: '#CB5229' }, { l: 'High', c: '#C8980E' }, { l: 'Medium', c: '#1AABBA' }, { l: 'Low', c: '#94A3B8' }].map(s => (
                  <div key={s.l} className="flex items-center gap-1.5">
                    <div className="w-2 h-2 rounded-full" style={{ backgroundColor: s.c }} />
                    <span style={{ color: '#94A3B8', fontSize: 11 }}>{s.l}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          <CardContent>
            <ResponsiveContainer width="100%" height={200}>
              <AreaChart data={ALERT_TREND_DATA} margin={{ top: 5, right: 5, left: -20, bottom: 0 }}>
                <defs>
                  {[{ id: 'critical', c: '#CB5229' }, { id: 'high', c: '#C8980E' }, { id: 'medium', c: '#1AABBA' }, { id: 'low', c: '#94A3B8' }].map(g => (
        ...
        <Card>
          ...
          <CardContent>
            <ResponsiveContainer width="100%" height={140}>
              <PieChart>
                <Pie data={SOURCE_DISTRIBUTION} cx="50%" cy="50%" innerRadius={44} outerRadius={66} dataKey="value" paddingAngle={3}>
                  {SOURCE_DISTRIBUTION.map((entry, i) => (
            ...
            <div className="space-y-2 mt-1">
              {SOURCE_DISTRIBUTION.slice(0, 5).map((src) => (
                <div key={src.name} className="flex items-center gap-2">
                  <div className="w-2 h-2 rounded-full shrink-0" style={{ backgroundColor: src.color }} />
                  <span style={{ color: '#64748B', fontSize: 11, flex: 1 }}>{src.name}</span>
                  <span style={{ color: '#0F172A', fontSize: 11, fontWeight: 600 }}>{src.value}%</span>
                </div>
              ))}
            </div>
          </CardContent>
      ...
      {/* ── Alerts by Client + Client Health ── */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <Card className="lg:col-span-2">
          ...
          <CardContent>
            <ResponsiveContainer width="100%" height={190}>
              <BarChart data={CLIENT_ALERT_COUNTS} margin={{ top: 5, right: 5, left: -20, bottom: 0 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="#F1F5F9" />
          ...
          <CardContent className="space-y-4">
            {Object.values(CLIENTS).map((client) => (
              <div key={client.id}>
                <div className="flex items-center justify-between mb-1.5">
                  ...
                  <div className="flex items-center gap-1.5">
                    ...
                    <span style={{
                      ...
                      color: client.status === 'healthy' ? '#10B981' : client.status === 'warning' ? '#C8980E' : '#CB5229',
                    }}>
                      {client.status}
                    </span>
                  </div>
                ...
                <Progress
                  value={Math.min(100, (client.openAlerts / 50) * 100)}
                  indicatorClassName={undefined}
                  style={{ height: 5 }}
                />
                <p style={{ color: '#CBD5E1', fontSize: 10, marginTop: 3 }}>
                  {client.openAlerts} open alerts
                </p>
              </div>
            ))}
          </CardContent>
        ...
        <Card className="lg:col-span-2 overflow-hidden">
          ...
          <div className="mt-3">
            ...
            <div className="grid px-5 py-2 bg-slate-50" style={{ gridTemplateColumns: '1fr 80px 90px 60px', borderTop: '1px solid #F1F5F9', borderBottom: '1px solid #F1F5F9' }}>
              {['Alert', 'Severity', 'Client', 'Time'].map(h => (
                <span key={h} style={{ color: '#94A3B8', fontSize: 10, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em' }}>{h}</span>
              ))}
            </div>
              ...
              return (
                <div
                  ...
                  style={{
                    ...
                    borderBottom: i < recentAlerts.length - 1 ? '1px solid #F8FAFC' : 'none',
                    borderLeft: `3px solid ${client.color}`,
                  }}
                >
                  <div className="min-w-0 pr-2">
                    <p style={{ color: '#334155', fontSize: 12, fontWeight: 500, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                      {alert.title}
                    </p>
          ...
          <CardHeader className="pb-0">
            <div className="flex items-center justify-between">
              ...
              <span
                ...
                style={{ backgroundColor: '#FBF0EC', color: '#CB5229', minWidth: 24 }}
              >
          ...
          <CardContent className="pt-3 space-y-2">
            {openIncidents.slice(0, 6).map((inc) => {
              ...
              return (
                ...
                >
                  <p style={{ color: '#334155', fontSize: 12, fontWeight: 500, lineHeight: 1.35, marginBottom: 6 }}>
                    {inc.title}
                  </p>
